...
- (void)setup {
    ...
    mask.strokeColor = [[UIColor colorWithRed:0.62 green:0.16 blue:0.16 alpha:1.0]CGColor];
    mask.lineWidth = 2;
...
- (void)goToCoachMarkIndexed:(NSUInteger)index {
    ...
    if (self.enableContinueLabel) {
        if (markIndex == 0) {
            ...
            lblContinue = nil;
            btnPrevious = [[UIButton alloc] initWithFrame:(CGRect){{0, self.bounds.size.height - 30.0f}, {btnSkipWidth, 30.0f}}];
            ...
            btnPrevious.titleLabel.font = [UIFont boldSystemFontOfSize:13.0f];
            btnPrevious.alpha = 0.0f;
            btnPrevious.tintColor = [UIColor whiteColor];
