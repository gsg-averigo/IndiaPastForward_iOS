...
- (instancetype)init{
    ...
    k =[[NSUserDefaults standardUserDefaults]integerForKey:@"Share"];
...
-(void)setCloseButtonType:(KGModalCloseButtonType)closeButtonType {
    ...
    if(closeButtonType == KGModalCloseButtonTypeNone){
        ...
        if(closeButtonType == KGModalCloseButtonTypeLeft){
...
- (void)showWithContentView:(UIView *)contentView andAnimated:(BOOL)animated 
{
    ...
    NSLog(@" share value %ld" ,(long)[[NSUserDefaults standardUserDefaults]integerForKey:@"Share"]);
    if ([[NSUserDefaults standardUserDefaults]integerForKey:@"Share"] ==100)
    {
    ...
    else
    {
        if (IPAD)
        {
            containerViewRect.origin.x = -200;
            containerViewRect.origin.y = 400;
                     }
        else
        {
            containerViewRect.origin.x = 50;
...
//    styleLayer.borderColor = [[UIColor colorWithRed:0.784 green:0.447 blue:0.38 alpha:1] CGColor];
...
- (void)setModalBackgroundColor:(UIColor *)modalBackgroundColor{
    if(_modalBackgroundColor != modalBackgroundColor){
        _modalBackgroundColor = modalBackgroundColor;
        self.styleLayer.backgroundColor =(__bridge CGColorRef)([UIColor whiteColor]);
    }
}
